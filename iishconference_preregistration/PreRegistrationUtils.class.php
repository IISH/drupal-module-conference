...
class PreRegistrationUtils {
	...
	public static function getParticipantTypesForUser() {
		...
		foreach (CachedConferenceApi::getParticipantTypes() as $participantType) {
			if (!$participantType->getWithPaper() &&
				(array_search($participantType->getId(), $typesToShow) !== false)
			) {
   ...
   *
   * @return PaperCoAuthorApi The paper co-author for a given session and co-author
   */
