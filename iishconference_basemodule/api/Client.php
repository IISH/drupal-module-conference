    ...
    */
    ...
    public function __construct($client_id, $client_secret, $client_auth = self::AUTH_TYPE_URI, $certificate_file = null)
    {
        ...
        if (!empty($this->certificate_file)  && !is_file($this->certificate_file)) {
            throw new InvalidArgumentOAuthException('The certificate file was not found', InvalidArgumentOAuthException::CERTIFICATE_NOT_FOUND);
        }
    }
    ...
    {
        if (!$grant_type) {
            throw new InvalidArgumentOAuthException('The grant_type is mandatory.', InvalidArgumentOAuthException::INVALID_GRANT_TYPE);
        }
        $grantTypeClassName = $this->convertToCamelCase($grant_type);
        $grantTypeClass = $grantTypeClassName;
        if (!class_exists($grantTypeClass)) {
            throw new InvalidArgumentOAuthException('Unknown grant type \'' . $grant_type . '\'', InvalidArgumentOAuthException::INVALID_GRANT_TYPE);
        }
        ...
        $rp = new ReflectionClass($grantTypeClass);
        $parameters['grant_type'] = $rp->getConstant('GRANT_TYPE');
        $http_headers = array();
            ...
            switch ($this->access_token_type) {
                case self::ACCESS_TOKEN_URI:
                    if (is_array($parameters)) {
                        ...
                        throw new InvalidArgumentOAuthException(
                            'You need to give parameters as array if you want to give the token within the URI.',
                            InvalidArgumentOAuthException::REQUIRE_PARAMS_AS_ARRAY
                        );
                    }
                    break;
    ...
    private function convertToCamelCase($grant_type)
    {
        ...
        array_walk($parts, create_function('&$item', '$val = $item = ucfirst($item);'));
...
class InvalidArgumentOAuthException extends OAuthException
{
