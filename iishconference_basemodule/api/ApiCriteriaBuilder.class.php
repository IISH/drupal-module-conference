...
class ApiCriteriaBuilder {
	...
	private function addCriteria($property, $value, $method = null) {
		...
		if (in_array(trim($method), self::$allowedMethods)) {
			$this->criteria[] =
				array('property' => trim($property), 'value' => trim($value), 'method' => trim($method));
		}
		else if (in_array(trim($property), self::$otherProperties)) {
			$this->criteria[] = array('property' => trim($property), 'value' => trim($value), 'method' => $method);
		}
	}
}