		...
		<tr>
			...
			<td class="program">
				<?php if (isset($_GET['paper'])) : ?>
					...
					<?php if (!is_null($variables['paper']->getCoAuthors()) && (strlen($variables['paper']->getCoAuthors()) > 0)) : ?>
				...
				<?php else : ?>
					<?php foreach ($variables['program'] as $i => $session) : ?>
						...
						<strong>
							...
							<?php print $session['sessionCode']; ?>: <?php print $variables['highlight']->highlight($session['sessionName']); ?>
						</strong>
						...
						<table class="program">
							...
							<tr>
								<td width="50%" class="program">
									<?php print (count($session['networks']) > 1) ? t('Networks:') : t('Network:'); ?>

									<?php foreach ($session['networks'] as $j => $network) : ?>
										<a href="?network=<?php print $network['networkId']; ?>"><?php print $network['networkName']; ?></a><?php if (count($session['networks']) !== $j+1) : ?>, <?php endif; ?>
									<?php endforeach; ?>
								</td>
								...
								<?php foreach ($variables['types'] as $j => $type) :
									...
									while ((count($participantsWithoutPaper) > 0) && ($participantsWithoutPaper[0]['type'] == $type->getType())) {
									...
									if (count($participants) == 0) {
										...
										print $type . ': ' . $variables['highlight']->highlight($participants[0]['participantName']);
									}
									else {
										$names = array();
						...
						<?php if ((($i+1) < count($variables['program'])) && ($session['timeId'] != $variables['program'][$i+1]['timeId'])) : ?>
							<hr class="program_hr" />
						<?php endif; ?>
					<?php endforeach; ?>
				<?php endif; ?>
